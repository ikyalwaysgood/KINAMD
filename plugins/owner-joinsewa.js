let handler = async (m, { conn, text, usedPrefix, args, participants }) => {
  ...
  var name = m.sender
  var fkonn = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(m.chat ? { remoteJid: '6285794908894@s.whatsapp.net' } : {}) }, message: { contactMessage: { displayName: `${await conn.getName(name)}`, vcard: `BEGIN:VCARD\nVERSION:3.0\nN:;a,;;;\nFN:${name}\nitem1.TEL;waid=${m.sender.split('@')[0]}:${m.sender.split('@')[0]}\nitem1.X-ABLabel:Ponsel\nEND:VCARD`}}}
...
Thanks dah invite *${m.name}*`, fkonn, {
    mentions: d
     ...
     await conn.reply(res, `Tapi Boong:v`, 0)
